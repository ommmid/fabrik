...
BOOST_AUTO_TEST_CASE(EmptyChain)
{
    ...
    BOOST_CHECK_THROW(fabrik::RobotState(chain, base), std::exception);
}
...
BOOST_AUTO_TEST_CASE(RobotStateConstructor)
{
    ...
    fabrik::RobotStatePtr fabrik = 
        std::make_shared<fabrik::RobotState>(maker.chain, maker.base);
     
    fabrik->printState("Robot State Constructor", std::vector<int>{-1});
}
...
BOOST_AUTO_TEST_CASE(updateStateTarget)
{
    ...
    fabrik->setReachingDirection(fabrik::ReachingDirection::FORWARD);
    Eigen::Affine3d target(Eigen::AngleAxisd(M_PI/7, Eigen::Vector3d::UnitZ()));

    BOOST_CHECK_THROW(fabrik->updateState(target), std::exception);
}
...
BOOST_AUTO_TEST_CASE(updateState)
{
    ...
    fabrik->printState("update state test - initial - forward completed", std::vector<int>{-1});

    // one backward reaching
    fabrik->setReachingDirection(fabrik::ReachingDirection::BACKWARD);

    fabrik->updateState(0.3, 2);
    fabrik->printState("update state test - update J_2 - backward", std::vector<int>{-1});

    fabrik->updateState(0.4, 1);
    fabrik->printState("update state test - update J_1 - backward", std::vector<int>{-1});

    BOOST_CHECK_THROW(fabrik->updateState(0.5, 0), std::exception);
    fabrik->printState("update state test - update J_0 - backward", std::vector<int>{-1});

    // one forward reaching
    fabrik->setReachingDirection(fabrik::ReachingDirection::FORWARD);

    fabrik->updateState(0.6, 0);
    fabrik->printState("update state test - update J_0 - forward", std::vector<int>{-1});

    fabrik->updateState(0.45, 1);
    fabrik->printState("update state test - update J_1 - forward", std::vector<int>{-1});

    fabrik->updateState(0.26, 2);
    fabrik->printState("update state test - update J_2 - forward", std::vector<int>{-1});
}
...
BOOST_AUTO_TEST_CASE(ForwardKinematics2D)
{
    ...
    for (int k = 0; k < 3; ++k)
        fabrik->updateState(fk_joints_values[k], k);
    
    Eigen::Affine3d end_effector = fabrik->getFrames(2).second;

    // fabrik->printState("test/fabrik.cpp : FK 2D", std::vector<int>{-1});
